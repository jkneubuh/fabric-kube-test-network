...
function do_enroll() {
  ...
  local ca_url=https://${user}:${pazz}@${org}-ca-ca.${INGRESS_DOMAIN}
...
function appear_as() {
  ...
  export CORE_PEER_ADDRESS=${org}-${peer}-peer.${INGRESS_DOMAIN}:443
  export CORE_PEER_LOCALMSPID=${mspid}
  export CORE_PEER_MSPCONFIGPATH=$PWD/organizations/${org}/enrollments/${org}admin/msp
  export CORE_PEER_TLS_ENABLED=true
  export CORE_PEER_TLS_ROOTCERT_FILE=$PWD/channel-config/organizations/peerOrganizations/${INGRESS_DOMAIN}/msp/tlscacerts/tlsca-signcert.pem
  export CORE_PEER_CLIENT_CONNTIMEOUT=15s
  export CORE_PEER_DELIVERYCLIENT_CONNTIMEOUT=15s

  export ORDERER_ENDPOINT=org0-orderernode1-orderer.trader1.fyre.ibm.com:443
  export ORDERER_TLS_CERT=${PWD}/channel-config/organizations/ordererOrganizations/trader1.fyre.ibm.com/orderers/orderernode1/tls/signcerts/tls-cert.pem
}
